...
fetch('json/skillinfo/en.json?16').then((response) => response.json()).then((json) => {
    charlist = json;
...
fetch('json/skillinfo/localisation_en.json?16').then((response) => response.json()).then((json) => {
    skillbuffnames["en"] = json;
});

if (data?.language) {
    if (data.language != "EN" && data.language != "Id") {
        fetch('json/skillinfo/' + data.language.toLowerCase() + ".json?16").then((response) => response.json()).then((json) => {
            chartranslate = json;
            if (nameReady) {
        ...
        fetch('json/skillinfo/localisation_' + data.language.toLowerCase() + '.json?16').then((response) => response.json()).then((json) => {
